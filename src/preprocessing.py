...
def create_length_distributions(db_path, cores = False, db_prefix = False, qnames = False):
	...
	for df2 in shared_dic_list[0]:
		shared_lengths = shared_lengths.add(df2, fill_value=0)
	for df2 in fetal_dic_list[0]:
		fetal_lengths = fetal_lengths.add(df2, fill_value=0)

	if qnames:
		with open('shared_qnames_list.txt', 'w') as f:
			for q in shared_qnames:
				print(q, file = f)
		with open('fetal_qnames_list.txt', 'w') as f:
			for q in fetal_qnames:
				print(q, file = f)
